...
const service = axios.create({
  baseURL: process.env.SAAS_API, // saasApi的base_url
  ...
  err => {
    ...
    return backoff.then(function() {
      if (err.message.indexOf('401') > -1) {
        return axios.get(process.env.BASE_LOGIN_API + '/api/auth/token', { headers: { 'X-Authorization': 'Bearer ' + getRefreshToken() }}).then(response => {
