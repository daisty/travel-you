<template>
  <div class="app-container">
    <el-form ref="orderInfoForm" :rules="rules" :model="orderInfoForm" label-width="120px">
      <div class="title">
        <span class="fontUser"> 请选择分销商</span><span class="headStyle"></span>
      </div>
      <el-card class="box-card margin-bottom-15">
        <div class="text item">
          <el-form-item prop="hotelId" class="textSize leftHun">
            <strong>{{$t('hotelOrder.distributor')}}:</strong>
            <el-input class="inputSer" size="small" v-model="keyName"
                      :placeholder="$t('hotelOrder.choosingDistributor')" disabled="true"></el-input>
            <el-button type="primary" size="mini" @click="transfer()">{{$t('hotelOrder.choosingDistributor')}}
            </el-button>
          </el-form-item>
        </div>
      </el-card>
    ...
    <!-- 转移弹窗 -->
    <el-dialog :title="$t('hotelOrder.choosingDistributor')" class="TwoFontSize"
               :visible.sync="transferDialogFormVisible">

      <div class="list-filter tenBottom">
        <div class="tenBottom tenTop">
          <strong>商户所在地: </strong>
          <el-select size="small" class="select" :placeholder="$t('inventoryTransshipment.province')"
                     v-model="queryParam.province">
          </el-select>
          <el-select size="small" class="select" :placeholder="$t('inventoryTransshipment.city')"
                     v-model="queryParam.city">
          </el-select>
          <strong class="padLeftTwo">商户等级: </strong>
        ...
        <div class="tenBottom">
          <strong>商户关键字: </strong>
          <el-input class="widthTwoFour" size="small" v-model="distributor.nameKey"
                    :placeholder="$t('hotelList.syncKey')"></el-input>
          <span class="search_text TwoRight">(单位名称、姓名、手机号码、ID、用户登录名)</span>
      ...
      <el-table :data="distributor.tableData" v-loading="transferLoading" border fit
        ...
        <el-table-column label="账户余额">
          <template slot-scope="scope">
            {{scope.row.distributorTotalMoney}}
          </template>
        </el-table-column>
        <el-table-column label="采购情况">
          <template slot-scope="scope">
            {{scope.row.distributorTotalOrder}}
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页内容 -->
      <div class="pagination-box text-right tenTop">
        <el-pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      distributorList() {
        const data = {
          resultNum: this.distributor.resultNum,
          page: this.distributor.currentPage
        }
        merchantInfo.list(data).then(res => {
          if (res.results && res.results.length && res.sizeAll !== '0') {
            this.distributor.tableData = res.results
            this.distributor.total = res.sizeAll
          } else {
            this.distributor.tableData = null
            this.bindHotel.total = res.sizeAll
          }
        })
      },
      // 获取所有
      ...
      saveTransfer() {
        this.keyName = this.distributor.tableData[this.radio].mchName
        this.distributorId = this.distributor.tableData[this.radio].id
      },
